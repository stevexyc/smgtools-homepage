...
<section class="col-lg-4 col-sm-4 eventscol">
	<h2>Undergrad Events</h2>
	<?php
		$events = @file_get_contents("https://smgapps.bu.edu/smgnet/smgcalendarU/todayjson_smgworld_upo.cfm");
		if($events != false) {
	...
	<article>
		...
		<?php } ?>
			<h3><?=h($event["TITLE"])?></h3></a>
				<time><?=h($newDate)?></time>
		<?php if($event["LOCATION"] != "BU") { ?>
			<p><?=h($event["LOCATION"])?></p>
		<?php } ?>
		<?php if($event["WEBLINK"] != 'http://') { ?><?php } ?>
	</article>
	<?php }
		}
	?>
	<a href="http://smgworld.bu.edu/upo-calendar/" target="_blank">Undergraduate Events >></a>
	<hr>
...
<section class="col-lg-8 col-sm-8">
    ...
    <div id="slider" class="nivoSlider">
    	<?php
$pageID = isset($_GET["page"]) ? $_GET["page"] : "undergrads";
...
<section class="col-lg-6 news-col">
	...
	<?php
	if(file_exists("upocache.htm") && filemtime("upocache.htm") > time()-300) {
		$nt= "<time class='newstime'>updated " . time_elapsed_string(filemtime("upocache.htm")) . "</time>";
		echo str_replace('<time class="newstime">updated just now</time>', $nt, file_get_contents("upocache.htm"));
	} else { ob_start(); ?>
	<h2>Life@SMG</h2>
	...
	<?php
	$feed = @simplexml_load_file("http://smgworld.bu.edu/upo/feed/");
	if($feed != false) {
...
<section class="col-lg-6 hourscol">
	...
	<?php
		...
		file_put_contents("upocache.htm",$newsOutput);
	}
	?>
</section>
</section>
